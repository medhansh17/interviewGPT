...
const MainAssessment = () => {
    ...
    useEffect(() => {
        const fetchTechnicalQuestions = async () => {
            const resp = await api.post('/fetch_technical_questions', { candidate_name: "JARED RICHARDS", job_id: 1 });
                                ...
                                {technicalQuestions.map((item: any, index: number) => {
                                    return (
                                        <li className="mb-2" key={item.id} onClick={() => setCurrentIndex(index)}>
                                            <button className={currentIndex == index ? `flex  items-center w-full p-2 bg-blue-400 text-white rounded-lg` : "w-full text-left p-2 bg-zinc-200 hover:bg-zinc-300 rounded"}>
