...
const codeQues=async()=>{
  const resp = await api.post("/fetch_coding_question", { candidate_name: canName, job_id: id });
  console.log(resp);
...
codeQues()
   ...
   const codeSubmit=async()=>{
    const resp=await api.post("/store_code_response",{ name: canName, job_id: id,question:codingQuestion.question, code:code})
...
<div className=" md:flex-row bg-white-600 text-zinc-900 dark:text-zinc-100 ">
  <div className='flex '>
  <div className="md:w-1/3 p-4 border-r border-zinc-300 dark:border-zinc-700" style={{    height: "80vh",
    overflowY: "scroll"}}>
    ...
    {/* <ol className="list-decimal ml-4">
      ...
      <li>
        ...
        the contents of <span className="italic">parameterVariable</span>.
                ...
                <div className="w-2/3 p-4"  style={{    height: "80vh",
    overflowY: "scroll"}}>
        ...
        <div className="space-x-2 flex" style={{justifyContent:"flex-end"}}>
                        <button className="bg-blue-500 text-white px-3 py-1 rounded">Run Code</button>
                        <button onClick={codeSubmit} className="bg-green-500 text-white px-3 py-1 rounded">Submit Code</button>
                    </div>       
</div> 
