...
const TechSkillTable: React.FC<TechSkillTableProps> = ({
  fetchSkill,
  role,
}) => {
  ...
  const generateQuestion = async () => {
    ...
    try {
      const transformedSkills = Object.entries(inputValues).map(
        ([softSkill, levels]) => ({
          skill: softSkill,
      ...
      const response = await api.post("/generate_technical_questions", {
        technical_skills: transformedSkills,
        name: role,
      });
      localStorage.setItem("techSkill", JSON.stringify(response?.data));
            ...
            <table className="border-collapse w-full md:rounded divide-y divide-gray-200 dark:divide-gray-700">
              <thead className="bg-blue-600 text-white">
                <tr>
                  <th
                    scope="col"
                    className="border border-blue-600 text-center p-2.5"
                  >
                    TechinalSkill
                  </th>
                  <th
                    scope="col"
                    className="border border-blue-600 text-center p-2.5 w-38"
                  >
                    <p>Basic Question</p>
              ...
              <tbody>
                {fetchSkill?.skills?.technical_skills.map(
                  (softSkill, index) => (
                    <tr key={index} className="even:bg-blue-100 odd:bg-blue-50">
