...
const TechSkillTable: React.FC<TechSkillTableProps> = ({ fetchSkill }) => {
    const initialInputValues: InputValues = Object.fromEntries(
        (fetchSkill?.skills?.technical_skills
            || []).map(skill => [skill, { basic: 0, intermediate: 0, advance: 0 }])
    );

    const [inputValues, setInputValues] = useState<InputValues>(initialInputValues);

    const handleInputChange = (softSkill: string, level: 'basic' | 'intermediate' | 'advance', value: number) => {
    ...
    const calculateTotal = (softSkill: string, level: 'basic' | 'intermediate' | 'advance'): number => {
                                        ...
                                        <tr key={index} className="border-b dark:border-neutral-500">
                                            ...
                                            <td className="whitespace-nowrap px-6 py-4">
                                                <input
                                                    ...
                                                    onChange={(e) =>
                                                        handleInputChange(softSkill, 'basic', parseInt(e.target.value, 10) || 0)
                                                    }
                                                />
                                            </td>
                                            <td className="whitespace-nowrap px-6 py-4">
                                                <input
                                                    ...
                                                    onChange={(e) =>
                                                        handleInputChange(softSkill, 'intermediate', parseInt(e.target.value, 10) || 0)
                                                    }
                                                />
                                            </td>
                                            <td className="whitespace-nowrap px-6 py-4">
                                                <input
                                                    ...
                                                    onChange={(e) =>
                                                        handleInputChange(softSkill, 'advance', parseInt(e.target.value, 10) || 0)
                                                    }
                                                />
                                            </td>
                                            <td className="whitespace-nowrap px-6 py-4">
                                                <span>{calculateTotal(softSkill, 'basic') + calculateTotal(softSkill, 'intermediate') + calculateTotal(softSkill, 'advance')}</span>
                                            </td>
                                        </tr>
                                    ))}
                            </tbody>
