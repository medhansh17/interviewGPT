...
const McqMain: React.FC<QuestionProps> = ({ questions, currentIndex, setCurrentIndex }: QuestionProps) => {
    ...
    const submitMcq = async () => {
        const payload = {
            ...
            job_id: id, // Replace with the actual job ID
            question: {
            ...
            {questions ? (
                <div className="p-4">
                    ...
                    <div className="mx-auto p-4">
                        ...
                        <form>
                            {Object.entries<string>(questions[currentIndex].options).map(([key, value]) => (
                        ...
                        <div className='flex' style={{justifyContent:"space-between"}}>
                            <div>
                                <button className="bg-blue-500 hover:bg-blue-600 text-white px-3 py-1 rounded-md" onClick={previousbtn}>PREVIOUS</button>
                                <button className="bg-green-500 ml-8 hover:bg-green-600 text-white px-3 py-1 rounded-md mr-2" onClick={nextbtn}>NEXT</button>
                            </div>
                            <div>
                                <button className={`bg-blue-500 text-white px-4 py-2 rounded ${allQuestionsAnswered ? '' : 'bg-gray-400 cursor-not-allowed'}`} disabled={!allQuestionsAnswered} onClick={submitMcq}>Submit</button>
                            </div>
                        </div>
                    </div>
                </div>
