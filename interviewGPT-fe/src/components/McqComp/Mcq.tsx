...
import SideBar from '../OnlineComp/SideBar';
...
useEffect(()=>{
  ...
  const behaveQues = async () => {
    const resp = await api.post('/fetch_technical_questions', { candidate_name: canName, job_id: id });
  ...
  return (
    <div className="flex h-screen">
          {technicalQuestions.length>0 &&  <><SideBar2 questions={technicalQuestions} currentIndex={currentIndex} setCurrentIndex={setCurrentIndex}/>
          <McqMain questions={technicalQuestions} setCurrentIndex={setCurrentIndex} currentIndex={currentIndex} /></>}
        </div>
